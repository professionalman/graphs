...
void memrecur(int i,int j,int arr[],vector<int>& p,vector<vector<int> >& myVector,vector<int>& p1,vector<vector<int> >& quant,int days_over[])
{
      //  memrecur(n,sum,arr,ss,myVector,qs,quant,days_over);
    ...
    {
        p.push_back(arr[i]);
        p1.push_back(days_over[i]);
        memrecur(i-1,j-arr[i],arr,p,myVector,p1,quant,days_over);
        int pos=0;
        for (int k =0;k< p.size();k++)
        {
            if(p[k]==arr[i])
    ...
    {
        memrecur(i-1,j,arr,p,myVector,p1,quant,days_over);
...
int main()
{
   gotoxy(1,2);
	...
	cout << "     **************************************************************" << endl;
        ...
        cout<<endl<<endl<<"Sub By:BHAVYA ARORA(17103119)"<<endl<<"  "<<"ABHISHEK DAS(17103128)"<<endl;
    delay(4000);
    system("cls");
    int cust_quantity[n1],sum_quantity=0,quantity,lo=0;
    printf("Enter the number of customers OR cities: ");
    scanf("%d",&n1);

     for(int i=0;i<n1;i++)
    {
        cout<<"\n\t Enter the quantity of GOODS required by Customer "<<i+1<<": ";
        cin>>quantity;
    ...
    cout<<"Please ..Enter Height of Single Truck(Refrigerator type structure i.e.double partition and multiple slabs)"<<"\n";
    int hei;
    cin>>hei;
    ...
    int days_over[]={0,25,15,45,66,89,24,70,99,53,15,87,23,65,43,68};
    int n=15;
    for(i=0;i<=n;i++)
    {
        for(j=i+1;j<=n;j++)
        {
            if(arr[i]>arr[j])
            {
                ...
                arr[i]=arr[j];
                arr[j]=temp;
                int temp1=days_over[i];
                days_over[i]=days_over[j];
                days_over[j]=temp1;
            }
        }
    }
...
//cout<<"FEFFSEQWSX";
    ...
    cout<<"\n\n\t\t IN ORDER TO CLEAR GODOWNS WITH MAXIMUM RETAINED STOCK"<<endl;

    for(int j=0;j<myVector[pos].size();j++)
    {
        cout<<"godown having capacity :"<<myVector[pos][j]<<"\n";
    }

    cout<<endl;
    takeInput();
    mincost(0); //passing 0 because starting vertex
    cout<<"\n\n\tMinimum cost is of transportation for ALEX "<<cost<<endl;
    ...
    A:
    cout<<"\n\tTotal cost of GOODS for customer"<<z+1<<": "<<amt;
    ...
    cout<<"\nAmount to be returned by ALEX:"<<ret<<endl;

        cout<<"\n----ALEX should return the following denomination(s) to the customer---- ";
